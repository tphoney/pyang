...
def pyang_plugin_init():
    plugin.register_plugin(Puppet3_providerPlugin())

class Puppet3_providerPlugin(plugin.PyangPlugin):
    def add_output_format(self, fmts):
        self.multiple_modules = True
        fmts['puppet3_provider'] = self

    def add_opts(self, optparser):
        optlist = [
            optparse.make_option("--puppet3_provider-help",
                                 dest="puppet3_provider_help",
                                 action="store_true",
                                 help="Print help on puppet3 symbols and exit"),
            optparse.make_option("--puppet3_provider-depth",
                                 type="int",
                                 dest="puppet3_provider_depth",
                                 help="Number of levels to print"),
            optparse.make_option("--puppet3_provider-path",
                                 dest="puppet3_provider_path",
                                 help="Subtree to print"),
            optparse.make_option("--puppet3_provider-print-groupings",
                                 dest="puppet3_provider_print_groupings",
                                 action="store_true",
                                 help="Print groupings"),
            ]
        g = optparser.add_option_group("puppet3_provider output specific options")
        g.add_options(optlist)

    def setup_ctx(self, ctx):
        if ctx.opts.puppet3_help:
            print_help()
            sys.exit(0)
    ...
    def emit(self, ctx, modules, fd):
        if ctx.opts.puppet3_path is not None:
            path = ctx.opts.puppet3_path.split('/')
            if path[0] == '':
                path = path[1:]
        else:
            path = None
        emit_puppet3(ctx, modules, fd, ctx.opts.puppet3_depth, path)
...
def emit_puppet3(ctx, modules, fd, depth, path):
    for module in modules:
        ...
        if ctx.opts.puppet3_print_groupings and len(module.i_groupings) > 0:
            if not printed_header:
                print_header()
                printed_header = True
            fd.write("  groupings:\n")
...
#        import ipdb; ipdb.set_trace()
        ...
        if s.parent.parent == None:
          fd.write("<" + name + " xmlns=\"" + module.search_one('namespace').arg + "\">")
